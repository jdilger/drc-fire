...
class step2:
    ...
    def ppZP(self,img):
      ...
      d = ee.Date(img.get('system:time_start'))
         #   todo check self.analysisPeriod, not sure if it should be pulling from img
      endDate = d.advance(ee.Number(self.analysisPeriod).divide(2),'day')
      startDate = ee.Date.fromYMD(d.get('year'),1,1)
      startYMD = ee.Number(self.getYearMonthDay(startDate.get('year'),startDate.get('month'),startDate.get('day')))
      endYMD = ee.Number(self.getYearMonthDay(endDate.get('year'),endDate.get('month'),endDate.get('day')))
    ...
    def export_burn_yearly(self,image, geometry=None, export_path=None,exportScale=None,crs=None,test=False):
      
        if exportScale is None:exportScale = exportScale
        if geometry is None: geometry = self.geometry.geometry()
    ...
    def main(self):
        ...
        joinZPandPPZP = self.joinCollections(zScoresTHandMasked.select(["burn","yearMonthDay","month","day"]), zScoresBurnMasked, False)

        burnYearly = joinZPandPPZP.select(['burn','yearMonthDay']).max()


        self.step1 = step1
        forExport = ee.Image.cat([burnYearly,self.cover.rename('landcover')]).set(metaData)
